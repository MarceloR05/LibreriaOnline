export const validateDireccionData = (req, res, next) => {
  const { id_usuario, calle, ciudad, codigo_postal, pais, es_predeterminada } = req.body;
  ...
  if (es_predeterminada !== undefined && typeof es_predeterminada !== 'boolean') {
    return res.status(400).json({ error: 'es_predeterminada debe ser un booleano' });
  }
